 ...
 * Published port.
 *
 * Maps a host port to an instance port so the instance can be reached from outside the unit.
 * Unlike exposed ports, which declare inter-instance accessibility, published ports create
 * a DNAT rule on the host. An empty host IP publishes on every local address.
 */
